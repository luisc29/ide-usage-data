...
def clustering_sessions_by_proportions(data, attributes, n_clusters, users, file_name, method='kmeans'):
    ...
    # compress the list users per cluster
...
def clustering(data):
    ...
    data_res = data[data['user'].isin(list_users)]
    users = data_res['user']
    users = users.unique()

    # clustering by proportions
    # kmeans
    clustering_sessions_by_proportions(data_res, TIME_SERIES_NAMES, 18, users, 'kmeans_centers.csv', 'kmeans')
    clustering_sessions_by_proportions(data_res, TIME_SERIES_NAMES, 0, users, 'affinity_centers.csv', 'affinity')
